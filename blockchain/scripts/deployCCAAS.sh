...
function deployCCAAS() {
    ...
    infoln "⚙️ Installing chaincode on peer0.org1..."
    installChaincode 1
    infoln "⚙️ Install chaincode on peer0.org2..."
    installChaincode 2
    ...
    successln "Chaincode successfully deployed!"
    exit 0
}
...
while [[ $# -ge 1 ]] ; do
  ...
  case $key in
    ...
    -ccaasdocker ) CCAAS_DOCKER_RUN="$2"; shift;;
...
## Verifications on user input
#User has not provided a name
if [ -z "$CC_NAME" ] || [ "$CC_NAME" = "NA" ]; then
  fatalln "No chaincode name was provided. Valid call example: ./deployCCAAS.sh -ccn basic -ccp ../asset-transfer-basic/chaincode-go "

# User has not provided a path
elif [ -z "$CC_SRC_PATH" ] || [ "$CC_SRC_PATH" = "NA" ]; then
  fatalln "No chaincode path was provided. Valid call example: ./deployCCAAS.sh -ccn basic -ccp ../asset-transfer-basic/chaincode-go "
